...
pmot_means <- pmot %>%
  ...
  summarise(
    Building_Maintenance_Scale       = mean(Building_Maintenance_Scale, na.rm = T),
    Building_Blight_Index            = mean(Building_Blight_Index, na.rm = T),
    Lawn_Maintenance_Scale           = mean(Lawn_Maintenance_Scale, na.rm = T),
    Lawn_Blight_Index                = mean(Lawn_Blight_Index, na.rm = T),
    General_Parcel_Maintenance_Scale = mean(General_Parcel_Maintenance_Scale, na.rm = T),
    General_Parcel_Blight_Index      = mean(General_Parcel_Blight_Index, na.rm = T)
  ) %>%
  ungroup() %>%
...
pmot_means_plots <- pmot_means %>% 
  ggplot(aes(x = as.numeric(Year), y = `Group Mean`, color = factor(Study_Condition))) + 
  geom_line() +
  geom_point()+
